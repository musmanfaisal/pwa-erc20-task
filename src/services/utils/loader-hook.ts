...
declare type invokeApiType = (args: { api: Function, callBack: Function, errorCallback: Function }) => Promise<void>;
export const useLoader = (initialState: boolean = false) => {
    ...
    const invokeApi: invokeApiType = async ({ api, callBack, errorCallback }) => {
        try {
            on();
